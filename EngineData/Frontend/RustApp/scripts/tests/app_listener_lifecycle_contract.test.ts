...
test("async listener installation cleans up immediately if App disposes during await", () => {
  ...
  assert.match(app, /const stop = await myVoiceBuildApi\.subscribeRuntime/);
  assert.ok((app.match(/if \(disposed\) \{\s*stop\(\);\s*return;\s*\}/g) ?? []).length >= 3);
});
...
test("mounted listeners still retain explicit teardown handles", () => {
  ...
  assert.match(app, /unlistenVoiceBuildRuntime = stop/);
