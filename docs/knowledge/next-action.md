...
Before TARGET_WINDOWS acceptance, obtain matching hosted source proof for the exact current Local SHA. Code Health now supports explicit `workflow_dispatch`; a manual run is classified as full-domain proof (Frontend + Rust + Python), produces an exact-SHA proof summary, and fails its aggregate gate if any required changed-domain job is not successful. Repository/quality/lock workflows also write exact-SHA summaries, and manual R3 Release Contract runs explicitly require the controlled Windows payload proof rather than silently stopping at source-only validation. If CI exposes a concrete regression, repair only that regression; do not reopen broad architecture expansion.
